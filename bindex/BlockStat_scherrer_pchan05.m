...
% previously known as BlockStatT85NewReversalTM_pchan.m
% TODO 05.m: label contiguous, block high, quick polyfit
% 50s to load data, 40s to run index
% 3s to load data, 10s to run index in kuang64 for 37 year
% 3s to load data, 5s to run index in kuang-intel for 37 year

% reversal only
% Based on Scherrer et al. 2006, a modified version of the Tibaldi and Molteni 1990 index

% For T42 interpolated grid

% Disregard and the first and last 4 days of data

% To load this blocking index:
%{
ver=['4a26_',caseid];  %NewReversal, bugfix+2
fn_load2 = ['../index_wise/BlockFreq_',ver,'.mat'];
mat_load2 = matfile(fn_load2);
PER4a26 = (mat_load2.PER);
%PER4a26 = (mat_load2.label>0);
%}
...
%% load and save data
%clearvars -except   text caseid season
tic;

season=true;
%thresh={0.01,'quantile',5,0,0}; caseid=['ERA-interim_19790101-20171231']; text=caseid;
%thresh={0.01,'quantile',5,5,0}; caseid=['ERA-interim_19790101-20171231']; text=caseid;  % TODO 49xx
...
prm.yS1  = prm.dphi+1;  % 73.95deg
prm.yS2  = find(latt42<-thresh{5},1,'last');  %18; %40.46deg  %16; % 46.04deg  %20; % 34.88deg  %25; % 20.93deg  %
...
%prm.GHGST = 0;
%prm.GHGNT = -10.0;  %Inf;  % ( m / deg lat ) %inf:ver+10
%prm.A = 0;
%prm.O = 0.5;
%prm.S
%prm.D = 5;
...
%strTitle = '4002: S06, 21-74N, Z500a, GHGNT=-10';
%strTitle = '4003: S06, 35-74N, Z500a, GHGNT=-10';
%strTitle = '4004: S06, 46-74N, Z500a, GHGNT=-10';
%strTitle = '4014: S06, 46-74N, Z500a, GHGNT=Inf';
%strTitle = '4022: S06, 21-74N, Z500a, GHGNT=-10';
%strTitle = '4023: S06, 35-74N, Z500a, GHGNT=-10';
%strTitle = '4922: S06, 21-74N, Z500a, GHGNT=-10';
%strTitle = '4923: S06, 35-74N, Z500a, GHGNT=-10';
%strTitle = '4933: S06, 35-74N, Z500a, GHGNT=Inf';
%strTitle = '4934: S06, 46-74N, Z500a, GHGNT=Inf';
%strTitle = '4a24: S06, 46-74N, Psi500a, GHGNT=-10';
%strTitle = '4a25: S06, 40-74N, D=1,GHGNT=-10';
%strTitle = '4a26: S06, 40-74N, D=5,GHGNT=-10';
...
%fn_z500a  = ['../index_wise/Z500_09xx_',caseid,'.mat'];  % TODO 49xx
%mat_z500a = matfile(fn_z500a);
%Wgt = mat_z500a.ZaDaily;  % already in latt42/lat0
%load(fn_z500a,'Wgt_jja','Wgt_djf');
...
%  Wgt = Wgt .* repmat(reshape(sign(latt42), [1 ds(2) 1]), [ds(1) 1 ds(3)]);
...
%%
%dy = mean(abs(latt42([yN1:yN2]+prm.dphi)-latt42(yN1:yN2)));
dy = abs(latt42([yN1:yN2]+prm.dphi)-latt42(yN1:yN2));
F_dy = repmat(reshape(dy, [1,length(yN1:yN2),1]), [ds(1),1,ds(3)]);
%disp('finish dy'); toc

F_rev = false(ds);
F_rev(:,yN1:yN2,:) = (VarDaily(:,yN1:yN2,:)-VarDaily(:,[yN1:yN2]-prm.dphi,:))./dy(1) >prm.GHGST;
F_rev(:,yS1:yS2,:) = (VarDaily(:,yS1:yS2,:)-VarDaily(:,[yS1:yS2]+prm.dphi,:))./dy(1) >prm.GHGST;
%disp('finish ss'); toc
%F_rev(:,yN1:yN2,:) = F_rev(:,yN1:yN2,:) & ( (VarDaily(:,[yN1:yN2]+prm.dphi,:)-VarDaily(:,yN1:yN2,:))/dy <prm.GHGNT );
%F_rev(:,yS1:yS2,:) = F_rev(:,yS1:yS2,:) & ( (VarDaily(:,[yS1:yS2]-prm.dphi,:)-VarDaily(:,yS1:yS2,:))/dy <prm.GHGNT );
 F_rev(:,yN1:yN2,:) = F_rev(:,yN1:yN2,:) & ( (VarDaily(:,[yN1:yN2]+prm.dphi,:)-VarDaily(:,yN1:yN2,:))./F_dy <prm.GHGNT );
 F_rev(:,yS1:yS2,:) = F_rev(:,yS1:yS2,:) & ( (VarDaily(:,[yS1:yS2]-prm.dphi,:)-VarDaily(:,yS1:yS2,:))./F_dy(:,end:-1:1,:) <prm.GHGNT );
  clear VarDaily F_dy;
%disp('finish f_rev'); toc

RA_D = zeros(ds,'uint8');
RA_D(:,:,1) = uint8( F_rev(:,:,1) );
for t = 2:ds(3)
    RA_D(:,:,t) = (RA_D(:,:,t-1)+1).* uint8(F_rev(:,:,t));
end
for t = ds(3)-1 : -1 :1
    RA_D(:,:,t) = max(RA_D(:,:,t),RA_D(:,:,t+1)).* uint8(F_rev(:,:,t));
end
%disp('finish RA_D'); toc
...
Wgt_jja(Wgt_jja<0)=0;  % TODO bugfix+20
Wgt_djf(Wgt_djf<0)=0;
...
%{
%disp('pause');pause
mat_2a70 = matfile(['../index_wise/BlockFreq_','2a70_',caseid,'.mat']);
%load(['../index_wise/BlockFreq_','2a70_',caseid,'.mat'],'PER_jja');
%  PERfreq_jja = mean(mat_2a70.PER_jja,3);
%Scale_jja = mean(mat_2a70.PER_jja,3)./mean(PER_jja,3);
%Scale_jja = repmat(mean(mat_2a70.PER_jja,3)./mean(PER_jja,3),[1 1 ds_jja(3)]);
%Scale_jja = repmat(mean(Hot_jja,3)./mean(PER_jja,3),[1 1 ds_jja(3)]);
Scale_jja = repmat( 1./std(mean(reshape(PER_jja,[ds(1:2),nd_jja,nyr]),3),[],4),[1 1 ds_jja(3)]);
%Scale_jja = repmat( std(mean(reshape(mat_2a70.PER_jja,[ds(1:2),nd_jja,nyr]),3),[],4)./std(mean(reshape(PER_jja,[ds(1:2),nd_jja,nyr]),3),[],4),[1 1 ds_jja(3)]);
%Scale_jja = repmat( std(mean(reshape(Hot_jja,[ds(1:2),nd_jja,nyr]),3),[],4)./std(mean(reshape(PER_jja,[ds(1:2),nd_jja,nyr]),3),[],4),[1 1 ds_jja(3)]);
Scale_jja(~isfinite(Scale_jja)) = 0;
ScaleX_jja = repmat( 1./std(mean(reshape(Hot_jja,[ds(1:2),nd_jja,nyr]),3),[],4),[1 1 ds_jja(3)]);
%ScaleX_jja = repmat( std(mean(reshape(PER_jja,[ds(1:2),nd_jja,nyr]),3),[],4)./std(mean(reshape(Hot_jja,[ds(1:2),nd_jja,nyr]),3),[],4),[1 1 ds_jja(3)]);
ScaleX_jja(~isfinite(ScaleX_jja)) = 0;
...
colormap(gca,flipud(hot(10))); caxis([0 111]); colorbar; caxis auto;  % TODO
...
%Hot_n = [areaEarth * squeeze(mean(mean(reshape(Hot_jja.*ScaleX_jja,[ds(1:2),nd_jja,nyr]),3),1))]';
...
%PERjja_n = [areaEarth * squeeze(mean(mean(reshape(PER_jja.*Scale_jja,[ds(1:2),nd_jja,nyr]),3),1))]';
...
%PERljja_n = [areaEarth * squeeze(mean(mean(reshape(PER_jja.*lsm_jja.*Scale_jja,[ds(1:2),nd_jja,nyr]),3),1))]';
PERlwjja_n = [areaEarth * squeeze(mean(mean(reshape(PER_jja.*lsm_jja.*Wgt_jja,[ds(1:2),nd_jja,nyr]),3),1))]';
disp(sprintf('jja: D=%i GHGNT=%.4f GHGST=%.4f dphi=%i %+.3f %+.3f %+.3f %+.3f',prm.D,prm.GHGNT,prm.GHGST,prm.dphi, corr(PERjja_n(:),Hot_n(:)), corr(PERljja_n(:),Hot_n(:)) , corr(PERwjja_n(:),Hotw_n(:)), corr(PERlwjja_n(:),Hotw_n(:)) ));
...
addpath('/n/home05/pchan/bin');
figure('units','inches','position',[0 1 11 8.5], 'paperUnits','inches','papersize',[11 8.5],'paperposition',[0 0 11 8.5]);
pcolorPH(-15:5:0, 1:8,rrr.r01);
%colormap(gca,flipud(hot)); colorbar;
colormap(gca,b2r(-1,1)); colorbar;
  for pm = 1:8
    for prma = -15:5:0
      text(prma,pm,sprintf('%+.3f',rrr.r01(pm,(prma+20)/5)),'HorizontalAlignment','center','fontsize',16);
    end
  end
...
hold on;
  xlabel({'Blocking area on land (km^2*day)'});
  ylabel({'Extreme area (km^2*day)'});
  cm=parula(nd_jja); f_cm = @(nBlock) cm(1+mod(PERjjaAttr.tmax(nBlock)-1,nd_jja),:);
  colormap(gca,cm); caxis([0.5 nd_jja+0.5]); colorbar;
  for nBlock = 1:numel(PERjjaAttr.id)
%    plot(xdata(nBlock),ydata(nBlock) ,'o','color',f_cm(nBlock));
   if (xdata(nBlock))
    text(double(xdata(nBlock)),double(ydata(nBlock)), PERjjaAttr.id{nBlock},'color',f_cm(nBlock),'HorizontalAlignment','center' );
...
title({'color: tmax'}, 'interpreter','none');
axis square tight; axis([min(xdata),max(xdata), min(ydata),max(ydata)]);
...
colormap(gca,flipud(hot(10))); caxis([0 2.5]); colorbar; %caxis auto;  % TODO
...
%axesm('MapProjection','pcarree','MapLatLimit',[0 90],'MapLonLimit',[-180 180]);
pcolormPC(latt42,lont42, 100*PERfreq_jja'); shading flat;
colormap(jet(12)); caxis([0 24]); colorbar; caxis auto;  % TODO fig2a in Scherrer 2006
plotm(coastlat,coastlon,'k')
...
%axesm('MapProjection','pcarree','MapLatLimit',[0 90],'MapLonLimit',[-180 180]);
pcolormPC(latt42,lont42, 100*PERfreq_djf'); shading flat;
colormap(jet(12)); caxis([0 24]); colorbar; caxis auto;  % fig2a in Scherrer 2006
plotm(coastlat,coastlon,'k')
...
%axesm('MapProjection','pcarree','MapLatLimit',[0 90],'MapLonLimit',[-180 180]);
pcolormPC(latt42,lont42, 100*PERfreq_jja'); shading flat;
colormap(jet(12)); caxis([0 24]*50); colorbar; caxis auto;
plotm(coastlat,coastlon,'k');
...
plotm(coastlat,coastlon,'k')
if (contains(textTH, 'wgt'))
  colormap(gca,b2r(-2e-4,2e-4)); %colorbar; caxis auto;  %TODO
  title({strTitle,['JJA blocking regressed on extreme hot area'],'unit*%/K/km^2'},'fontsize',16);
else
  colormap(gca,b2r(-2e-5,2e-5)); %colorbar; caxis auto;
...
plotm(coastlat,coastlon,'k')
if (contains(textTH, 'wgt'))
  colormap(gca,b2r(-8e-5,8e-5)); %colorbar; caxis auto;
...
if (contains(textTH, 'wgt'))
  ...
  colormap(gca,b2r(-0.6,0.6)); colorbar; %caxis auto;
